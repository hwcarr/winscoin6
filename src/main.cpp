...
uint256 hashGenesisBlock("0x086ab22b105e939e395b419a39dd16001b0a30c828a6e4009c059cbf8ce27b5c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Winstarrxcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Winstarrxcoin Signed Message:\n";
...
{
    // Winstarrxcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Winstarrxcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Winstarrxcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Winstarrxcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Winstarrxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbc65f0a8b3c23e73a596ddddd878df4d380cae9d2ef625a54f741f0d49581f24");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2/16/2021 - Bitcoin hits 50K for the first time!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043d9b5c0db23448fdcfa330a083584fc3fac8bec0cbff563c9aeb8b29335aeeb19dc29fb46b64703bc973539422252ad72b4a8408f74fc29b1dfedb72f70e156e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1614055080;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084980910;

        if (fTestNet)
        {
            block.nTime    = 1614055047;
            block.nNonce   = 385400907;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xad36fa9f12f3a148aed67ef06c11514fb2ecc52478c777a759433699cc8e9637"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb3, 0xda }; // Winstarrxcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WinstarrxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WinstarrxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WinstarrxcoinMiner(CWallet *pwallet)
{
    printf("WinstarrxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("winstarrxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running WinstarrxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WinstarrxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WinstarrxcoinMiner, pwallet));
}
